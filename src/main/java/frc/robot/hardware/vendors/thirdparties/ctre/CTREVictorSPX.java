...
public class CTREVictorSPX {
  ...
  private static final Logger logger = Logger.getLogger(CTREVictorSPX.class.getName());

  public static void logFaults(VictorSPX CTREVictorSPX) {
    runTest(() -> {
      String separator = "// ==========";
      Faults CTRE_0xF = new Faults();
      CTREVictorSPX.getFaults(CTRE_0xF);
  
      if(CTRE_0xF.hasAnyFault()) {
        System.out.println("Faults detected on TalonSRX ID " + CTREVictorSPX.getDeviceID());

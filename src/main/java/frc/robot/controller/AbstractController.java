...
public abstract class AbstractController {
  ...
  protected void stopDrivetrain() {
    db.drivetrain.set(InputMap.EDriveData.STATE, EDriveState.PERCENT_OUTPUT);
    db.drivetrain.set(InputMap.EDriveData.DESIRED_THROTTLE_PCT, 0.0);
    db.drivetrain.set(InputMap.EDriveData.DESIRED_TURN_PCT, 0.0);
  }
  ...
  protected void indexCargo() {
    ...
    } else if (mIsBallAdded && db.feeder.get(EFeederData.ENTRY_BEAM) == 1d) {
      mIsBallAdded = false;
    }
    // Removed exit beam
  ...
  public void activateFeeder() {
    db.feeder.set(EFeederData.STATE, EFeederState.PERCENT_OUTPUT);
    // Entry beam has been tripped and has not been untripped (ball has just hit
    // beam break but not entered system)
    if (mEntryGate.get() == XorLatch.State.XOR) {
      db.feeder.set(EFeederData.SET_FEEDER_pct, 0.4);
      mNumBalls++;
    }
    // Cargo has past the entrance and hasn't reached the exit beam yet and another
    // has entered
    ...
    // Cargo has past the entrance however there is only one ball so don't do
    // anything yet
    else if (mEntryGate.get() == XorLatch.State.BOTH && mExitGate.get() == XorLatch.State.NONE
        && mNumBalls == 1) {
    ...
    // Stop indexing if we have hit the exit beam (make sure no balls get shot out
    // prematurely)
  ...
  public void fireFeeder(double pSpeed, double pPulseSpeed) {
    ...
    if (mExitGate.get() == XorLatch.State.XOR) {
      ...
      if (mBallsShot == 0) {
        db.feeder.set(EFeederData.SET_VELOCITY_rpm, pSpeed);
      }
    ...
    } else if (db.feeder.get(EFeederData.EXIT_BEAM) == 1) {
      mExitGate.reset();
      mBallsShot++;
      mNumBalls--;
    }

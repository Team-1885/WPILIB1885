...
public class BaseAutonController extends AbstractController {
    ...
    public BaseAutonController() {
        ...
        mTimer = new Timer();
        mFirstLeg = new Timer();
        mDriveKinematics = new DifferentialDriveKinematics(ABC.feet_to_meters(REVLibCAN.TRACK_WIDTH_FEET));
        SmartDashboard.putNumber("trajectory-seconds",-1);


        mTrajectoryConfig = new TrajectoryConfig(1.0, 1.0);
        DifferentialDriveKinematics k = new DifferentialDriveKinematics(ABC.feet_to_meters(REVLibCAN.TRACK_WIDTH_FEET));
    ...
    public void execute() {
        ...
        data.add(ABC.feet_to_meters(db.drivetrain.get(InputMap.EDriveData.L_ACTUAL_POS_FT)));
        data.add(ABC.feet_to_meters(db.drivetrain.get(InputMap.EDriveData.R_ACTUAL_POS_FT)));
    ...
    protected void ramseteFollow(double curTime, double dT, DifferentialDriveWheelSpeeds actualSpeeds,
        ...
        db.drivetrain.set(InputMap.EDriveData.STATE, Enums.EDriveState.PATH_FOLLOWING_RAMSETE);
    ...
    private DifferentialDriveWheelSpeeds calculateActualSpeeds() {
        double actualLeftSpeedMeters = ABC.feet_to_meters(db.drivetrain.get(InputMap.EDriveData.L_ACTUAL_VEL_FT_s));
        double actualRightSpeedMeters = ABC.feet_to_meters(db.drivetrain.get(InputMap.EDriveData.R_ACTUAL_VEL_FT_s));
    ...
    private Pose2d getRobotPose() {
        double absX = initialState.poseMeters.getX() + db.drivetrain.get(InputMap.EDriveData.GET_X_OFFSET_METERS);
        double absY = initialState.poseMeters.getY() + db.drivetrain.get(InputMap.EDriveData.GET_Y_OFFSET_METERS);
        Rotation2d r2d = new Rotation2d(db.drivetrain.get(InputMap.EDriveData.ACTUAL_HEADING_RADIANS));
    ...
    private void updateDriveTrain(Pair<Double,Double> pOutput) {
        ...
        db.drivetrain.set(InputMap.EDriveData.L_DESIRED_VEL_FT_S, leftFeet);
        db.drivetrain.set(InputMap.EDriveData.R_DESIRED_VEL_FT_S, rightFeet);
    }

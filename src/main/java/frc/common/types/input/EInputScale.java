package frc.common.types.input;

/**
 * Enumeration that drives input scaling for any joystick. For example,
 * the following code will produce the same scale that we had in 2017 for
 * turning:<br>
 * <br>
...
public enum EInputScale {
   ...
   * @param pScalar        - the scalar to be applied. How it is applied depends
   *                       upon the enumeration itself.
   *                       USE SYSTEM SETTINGS to define constants for these
   *                       scalars.
  ...
  public double map(double pJoystickInput, double pScalar) {
    switch (this) {
      ...
      case SIN_WITH_FGAIN2:
        return Math.sin(Math.PI / 2 * pScalar * SIN_WITH_FGAIN1.map(pJoystickInput, pScalar))
            / Math.sin(Math.PI / 2 * pScalar);
      case SIN_WITH_FGAIN3:
        return Math.sin(Math.PI / 2 * pScalar * SIN_WITH_FGAIN2.map(pJoystickInput, pScalar))
            / Math.sin(Math.PI / 2 * pScalar);
      case PCT_SIN:
  ...
  /**
   * Since not all methods need a scalar, this utility method reduces some
   * boilerplate code.
   * 
   * @param pJoystickInput - the joystick, as-read from the codex
   * @return - the scaled equivalent of the joystick input

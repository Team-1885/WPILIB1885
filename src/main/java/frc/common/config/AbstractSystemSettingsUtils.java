...
public abstract class AbstractSystemSettingsUtils {
  ...
  public static void loadPracticeSettings(Settings sysSettings) {
  ...
  /**
   * This method will take all of the static fields in this class and apply the
   * values to the
   * passed in object's static fields that have the exact field name.
   * 
   * @param destinationObject
   *                          The object that will have it's static fields updated
   *                          with the fields in the from object
   */
  public static void copyOverValues(Object fromObject, Object destinationObject) {
    ...
    Map<String, Object> baseClassMap = Arrays.stream(allFields)
        .filter(aField -> Modifier.isStatic(aField.getModifiers()))
        .collect(Collectors.toMap(aField -> aField.getName(), aField -> {

          try {
            return aField.get(destinationObject);
          } catch (Exception e) {
            System.err
                .println("Unable to get field: " + aField.getName() + " from obj: " + destinationObject.getClass());
            e.printStackTrace();
            return null;
          }
        }));
    ...
    Arrays.stream(destinationObject.getClass().getFields()).filter(aField -> Modifier.isStatic(aField.getModifiers()))
        .forEach(aField -> {
          ...
          if (value != null) {
            try {
              ...
              System.err.println(
                  "Unable to set the field: " + aField.getName() + " on object: " + destinationObject.getClass());
              e.printStackTrace();
            }
          }
        });
